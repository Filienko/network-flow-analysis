...
run_bipartite_script() {
    ...
    ./"bipartite.sh" "$nodes_source" "$nodes_sink" "$maxProbability" "$minCapacity" "$maxCapacity" "$output_file" > /dev/null 2>&1
}
...
run_fixeddegree_script() {
    ...
    ./"fixeddegree.sh" "$nodes_source" "$edges" "$minCapacity" "$maxCapacity" "$output_file" > /dev/null 2>&1
}
...
run_mesh_script() {
    ...
    ./"mesh.sh" "$rows" "$columns" > "$output_file" > /dev/null 2>&1
}
...
run_random_script() {
    ...
    ./"random.sh" "$output_file" " " "$nodes_source" "$dense" "$maxCapacity" "$minCapacity" > /dev/null 2>&1
}
...
for nodes_source in $(seq $initial_nodes_source 250 $((initial_nodes_source + 250 * 2))); do
    for nodes_sink in $(seq $initial_nodes_sink 250 $((initial_nodes_sink + 250 * 2))); do
        for maxProbability in $(seq $initial_maxProbability 0.2 1); do
            for minCapacity in $(seq $initial_maxCapacity 50 $((initial_maxCapacity + 50 * 2))); do
              for maxCapacity in $(seq $initial_maxCapacity 50 $((initial_maxCapacity + 50 * 2))); do
...
for nodes_source in $(seq $initial_nodes_source 250 $((initial_nodes_source + 250 * 2))); do
    for edges in $(seq $initial_edges 250 $((initial_edges + 250 * 2))); do
        for minCapacity in $(seq $initial_minCapacity 50 $((initial_minCapacity + 50 * 2))); do
            for maxCapacity in $(seq $initial_maxCapacity 50 $((initial_maxCapacity + 50 * 2))); do
...
for rows in $(seq $initial_rows 10 $((initial_rows + 10 * 2))); do
    for columns in $(seq $initial_columns 10 $((initial_columns + 10 * 2))); do
